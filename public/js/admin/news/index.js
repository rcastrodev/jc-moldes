...
function dataSliderContent(content)
{
    ...
    form.reset()
    form.querySelector(`option[value="${content.content_3}"]`).setAttribute('selected', 'true')
    ...
    if (content.content_5) 
        form.querySelector('input[name="content_5"]').setAttribute('checked', 'on')
    else 
        form.querySelector('input[name="content_5"]').removeAttribute('checked')
}
